...
CREATE TABLE `__new_user_members` (
...
INSERT INTO `__new_user_members`("id", "user_id", "member_id", "invited_email", "invitation_id", "role", "status", "invited_by", "accepted_at", "revoked_at", "created_at", "updated_at") SELECT "id", "user_id", "member_id", "invited_email", "invitation_id", "role", "status", "invited_by", "accepted_at", "revoked_at", "created_at", "updated_at" FROM `user_members`;--> statement-breakpoint
DROP TABLE `user_members`;--> statement-breakpoint
ALTER TABLE `__new_user_members` RENAME TO `user_members`;--> statement-breakpoint
...
CREATE UNIQUE INDEX `uniq_user_members_invited_email` ON `user_members` (`invited_email`);